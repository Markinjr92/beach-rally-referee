...
const PublicTournamentView = () => {
  ...
  useEffect(() => {
    ...
    // Update title
    document.title = `${tournamentName} - VB Jukin`
    ...
    return () => {
      ...
      updateMetaTag('twitter:title', 'VB Jukin - Torneio de Vôlei de Praia', false)
  ...
  const handleShareLink = useCallback(() => {
    ...
    navigator.clipboard.writeText(currentUrl).then(() => {
      ...
      toast({
        title: 'Link copiado!',
        description: 'O link público do torneio foi copiado para a área de transferência.',
      })
    })
  }, [toast])
        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <div className="inline-flex items-center gap-2 rounded-full bg-white/10 border border-white/20 px-4 py-2">
              ...
              <span className="uppercase tracking-[0.2em] text-xs text-white/70">Torneio Público</span>
            </div>
            <Button
            ...
            <Card className="bg-slate-900/60 border border-white/20 text-white backdrop-blur-xl">
              <CardHeader className="space-y-4">
                ...
                <p className="text-sm text-white/70">
                  Acompanhe todos os confrontos e resultados do torneio em tempo real.
                </p>
                {sponsorLogos.length > 0 && (
